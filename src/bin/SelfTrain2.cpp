 ...
 * 
 *  Excutable file to self-train the classifiers with sample-based approach
 *  the granularity of labeling and selection of labeled data for the 
 *  set at each iteration is an argument sample.
 */
...
void selfTrain(ZMEClassifier &, vector<Sample *> &, vector<Sample *> &,
 ...
 */
 ...
 if (vTesting >= 1)
 {
  ...
  if (generateTestSamples(vcDevWSJSntncs, vFeatureSet, vDevWSJSyntDepFile, true, false, log) != 0)
   return -1;
  }
 ...
 if (vTesting >= 2)
 {
  ...
  if (generateTestSamples(vcTestWSJSntncs, vFeatureSet, vTestWSJSyntDepFile, true, false, log) != 0)
   return -1;
  }
 ...
 */
 ...
 zmeClassifier.train(vcLabeledSamples, 1, 1, true,
 ...
 */
 ...
 if (generateUnlabeledSamples(vcUnlabeledSntncs, vcUnlabeledSamples, vFeatureSet, 
 ...
 selfTrain(zmeClassifier, vcLabeledSamples, vcUnlabeledSamples, 
           ...
           vRemoveLabeled, vSTIterations, vLogSelection, vTesting, 
           vDevWSJOutput, vTestWSJOutput, vTestBrownOutput, log);
 ...
 * 
 *  - pUsePool determines that whether all unlabeled samples are labeled 
 *    in each iteration or a pool based on the number of iterations will
 *    be used
 *  - pSort indicates that all the unlabeled sentences should be sorted by
 *    ascending of sentence length before start. It is meaningful if pool
 *    is used.
 *  - pRemoveLabel indicates that each unlabeled data should be labeled
 *    only once, i.e. it should be removed from unlabeled data once labeled
 *  - pGrowthSize determines the number of newly labeled sentences that
 *    should be selected and added to the training set in each iteration 
 */
  ...
  while (1)
  {
   ...
   if (pRemoveLabeled)
    vAddedCount = moveToTraining(pTrainingSamples, pUnlabeledSamples, vPool,
                                 ...
                                 DEFAULT_SELF_TRAINING_CONVERGENCE_SAMPLE_COUNT,
   ...
   if (vAddedCount != 0)
   {
    ...
    mout("\nTraining " + intToStr(cntIteration) + "th classifier  (" + getLocalTimeStr() + ") ...\n",
         cout, pLog);
    pZMEClassifier.train(pTrainingSamples, 1, 1, false, 0, 
   ...
   mout("\nData used so far: " + intToStr(vDataUsedSofar) + " samples\n", cout, pLog);
...
{                            
 ...
 else
 {
  ...
  vSelected = selectAndMove(pPool, pTrainingSamples, pProbThreshold,
                            pNumberThreshold, pConvergenceThreshold,
                            pLogSelection, pLogFileName, pLog);
  
  switch (pPoolUsage)
  {
   // If no pool is used and no labeled sample was selected, self-training
   ...
   // If there was no sample selected (vSelected == 0), check if the
   // number of labeled data in this pool wasn't 0, then add to the end,
   // (instead of begining to prevent wandering around the same samples
                           ...
                           ofstream &pLog)
{
 ...
 for (; cntSortedPool > 0; cntSortedPool--)
  if ((pPool[cntSortedPool - 1]->getPrdArgLabelProb(1) >= pProbThreshold) &&
      (pPool[cntSortedPool - 1]->getPrdArgLabelProb(1) != 0))
   break;
 ...
 // a certain amount, it means that self-training has converged and there
 ...
 if (vSelectedCount != 0)
 {
  ...
  vMinProbability = pPool[vSelectedCount - 1]->getPrdArgLabelProb(1);
  vMaxProbability = pPool[0]->getPrdArgLabelProb(1);
