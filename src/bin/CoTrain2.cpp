/**
 *  @ RSK
 *  @ Version 0.1        24.03.2010
 * 
 *  Excutable file to co-train the classifiers with bothe common and 
 *  separate training sets, and sample-based approach
 * 
 */
...
short generateLabeledSamples(vector<Sentence *> &, vector<Sample *> &,
...
short generateTestSamples(vector<Sentence *> &, unsigned short [], 
                          string &, bool, bool, ofstream &);
short generateUnlabeledSamples(vector<Sentence *> &,  vector<Sample *> &,
                               unsigned short [], string &, bool, bool, ofstream &);
void coTrainCommon(vector<ZMEClassifier> &, vector<Sample *> &, vector<Sample *> &,
                   vector<Sentence *> &, vector<Sentence *> &, vector<Sentence *> &,
                   unsigned short, string &, double, bool, unsigned int, 
                   unsigned short, unsigned short, unsigned short,
                   double, unsigned short, bool, unsigned short, 
                   unsigned short, unsigned short, string &, 
                   string &, string &, ofstream &);
void coTrainSeparate(vector<ZMEClassifier> &, vector<Sample *> &, vector<Sample *> &,
                     vector<Sentence *> &, vector<Sentence *> &, vector<Sentence *> &,
                     unsigned short, string &, double, bool, unsigned int, 
                     unsigned short, unsigned short, unsigned short, 
                     double, unsigned short, bool, unsigned short, unsigned short, 
                     unsigned short, string &, string &, string &, ofstream &);
unsigned int addToTraining(vector<Sample *> &, vector<Sample *> &, 
                           unsigned int);
unsigned int moveToTraining(vector<Sample *> &, vector<Sample *> &, 
                            vector<Sample *> &, unsigned short, unsigned short,
                            unsigned short, unsigned short, double, unsigned short,
                            unsigned short, unsigned short, string, ofstream &);
unsigned int selectAndMoveConfident(vector<Sample *> &, vector<Sample *> &, 
                                    double, unsigned short, unsigned short, 
                                    unsigned short, string &, ofstream &);
unsigned int selectAndMoveAgreed(vector<Sample *> &, vector<Sample *> &, 
                                 unsigned short, unsigned short, 
                                 unsigned short, string &, ofstream &);
unsigned int selectAndMove(vector<Sample *> &, vector<Sample *> &, 
                           unsigned short, double, unsigned short, unsigned short,
                           unsigned short, string &, ofstream &);
short saveCoNLLProps(vector<Sentence *> &, unsigned short, string, ofstream &);
void setSelectedPrdLabeling(vector<Sentence *> &, unsigned short, ofstream &);
void setSelectedPrdLabeling(vector<Sample *> &, unsigned short, ofstream &);
...
const string ctmpStr  = "help";
 ...
 tmpLogFile += ".2.[" + getLocalTimeStr("YYYY-MM-DD-HH-MM") + "].log";
 ...
 */
 ...
 vector<Sample *> vcLabeledSamples;

 if (generateLabeledSamples(vcLabeledSntncs, vcLabeledSamples,
...
/**
 *  Samples are passed to the trainer
 * 
 ...
 */
 for (unsigned short cntView = 1; cntView <= FEATURE_VIEW_COUNT; cntView++)
 {
  ...
  zmeClassifiers[cntView - 1].train(vcLabeledSamples, cntView, cntView, true,
 ...
 */
 ...
 if (vTesting >= 1)
 {
  ...
  setSelectedPrdLabeling(vcDevWSJSntncs, CONFIDENECE_ONLY, log);
 ...
 if (vTesting >= 2)
 {
  ...
  setSelectedPrdLabeling(vcTestWSJSntncs, CONFIDENECE_ONLY, log);
 ...
 if (vTesting >= 3)
 {
  ...
  setSelectedPrdLabeling(vcTestBrownSntncs, CONFIDENECE_ONLY, log);
 ...
 */
 ...
 vector<Sample *> vcUnlabeledSamples;
 
 if (generateUnlabeledSamples(vcUnlabeledSntncs, vcUnlabeledSamples, vFeatureSets, 
                              vUnlabeledDataFile, false, false, log) != 0)
  return -1;
 ...
 if (vCoTrainingMethod == 1)
  coTrainCommon(zmeClassifiers, vcLabeledSamples, vcUnlabeledSamples, 
                vcDevWSJSntncs, vcTestWSJSntncs, vcTestBrownSntncs,
                vMEIterations, vMEPEMethod, vGaussian, vGlobalOpt, vPoolSize,
                vSelection, vCLSelMethod, vPoolUsage, vProbThreshold, 
                vNumberThreshold, vRemoveLabeled, vSTIterations, 
                vLogSelection, vTesting, vDevWSJOutput, vTestWSJOutput, 
                vTestBrownOutput, log);
 else if (vCoTrainingMethod == 2)
  coTrainSeparate(zmeClassifiers, vcLabeledSamples, vcUnlabeledSamples, 
                  vcDevWSJSntncs, vcTestWSJSntncs, vcTestBrownSntncs,
                  vMEIterations, vMEPEMethod, vGaussian, vGlobalOpt, vPoolSize,
                  vSelection, vCLSelMethod, vPoolUsage, vProbThreshold, 
                  vNumberThreshold, vRemoveLabeled, vSTIterations,  
...
{                                 
 ...
 vector<Sample *> vcPositiveSamples, vcNegativeSamples;
 ...
 if (oSampleGenerator.generateSamples(pSentences, 
                                      ...
                                      pWriteToFile) == 0)
 {
  pSamples.insert(pSamples.begin(), vcPositiveSamples.begin(), vcPositiveSamples.end());
  pSamples.insert(pSamples.end(), vcNegativeSamples.begin(), vcNegativeSamples.end());
...
{                                 
 ...
 if (oSampleGenerator.generateSamples(pSentences, pSamples, FEATURE_VIEW_COUNT,
 ...
 *  selection method (in two versions: sentence-based and sample-based)
 *  
 ...
 *  3- For training with common labels too, AGREEMENT_PREFERED_VIEW makes
 *     the output label exactly like the output of preferred view. So, it
 *     is almost not useful and may be removed in the future.
 *
 *  This sentence-based version is used for only 1st purpose
 */
                            ...
                            ofstream &pLog)
{
 for (vector<Sentence *>::iterator itSentence = pcLabeledSntncs.begin(); 
      ...
      itSentence++)
 {     
  (*itSentence)->setSelectedPrdLabels(pSelectionMethod, 0);
...
///---------------------------------------------------------------------
/**
 *  sample-based version of setSelectedPrdLabeling()
 *  (see above for comments)
 *  
 *  This sample-based version is used for the 2nd purpose
 */
void setSelectedPrdLabeling(vector<Sample *> &pcLabeledSamples, 
                            unsigned short pSelectionMethod,
                            ofstream &pLog)
{
 for (vector<Sample *>::iterator itSample = pcLabeledSamples.begin(); 
      itSample < pcLabeledSamples.end(); 
      itSample++)
 {     
  (*itSample)->setSelectedPrdLabel(pSelectionMethod, 0);
  cout << "\r" << (itSample - pcLabeledSamples.begin() + 1) << " samples processed" << flush;
  }
 pLog << pcLabeledSamples.size() << " samples processed" << flush;
...
void coTrainCommon(vector<ZMEClassifier> &pZMEClassifiers,
                   vector<Sample *> &pTrainingSamples,
                   vector<Sample *> &pUnlabeledSamples,
                   ...
                   ofstream &pLog)
{
 unsigned vSeedSize = pTrainingSamples.size();
 unsigned vAllUSize = pUnlabeledSamples.size();
 ...
 mout("Unlabeled Size: " + intToStr(pUnlabeledSamples.size()) + '\n', cout, pLog);
 ...
 // set as one pool
 if (pPoolUsage == 0)
  pPoolSize = pUnlabeledSamples.size();
 ...
 while (pUnlabeledSamples.size() > 0)
 {
  ...
  //   it is not fired
  if (pUnlabeledSamples.size() < pPoolSize)
   pPoolSize = pUnlabeledSamples.size();

  vPool.insert(vPool.end(),
               pUnlabeledSamples.begin(), 
               pUnlabeledSamples.begin() + pPoolSize);
  pUnlabeledSamples.erase(pUnlabeledSamples.begin(), 
                          pUnlabeledSamples.begin() + pPoolSize);
  ...
  while (1)
  {
   ...
   /**
    *  labeling unlabeled samples (sample-based)
    */
   ...
   setSelectedPrdLabeling(vPool, pCLSelMethod, pLog);
   ...
   if (pRemoveLabeled)
    vAddedCount = moveToTraining(pTrainingSamples, pUnlabeledSamples, vPool,
    ...
    vAddedCount = addToTraining(pTrainingSamples, vPool, vSeedSize);
   ...
   if (vAddedCount != 0)
   {
    ...
    for (unsigned short cntView = 1; cntView <= FEATURE_VIEW_COUNT; cntView++)
    {
     ...
     pZMEClassifiers[cntView - 1].train(pTrainingSamples, cntView, COMMON_VIEW, false, 0, 
    ...
    {
     mout("\nLabeling development data with selected common labels ...\n", cout, pLog);
     setSelectedPrdLabeling(pDevWSJSntncs, CONFIDENECE_ONLY, pLog);
     mout("\nLabeling development data is done!\n", cout, pLog);  
    ...
    {
     mout("\nLabeling WSJ test data with selected common labels ...\n", cout, pLog);
     setSelectedPrdLabeling(pTestWSJSntncs, CONFIDENECE_ONLY, pLog);
     mout("\nLabeling WSJ test data is done!\n", cout, pLog);  
    ...
    {
     mout("\nLabeling Brown test data with selected common labels ...\n", cout, pLog);
     setSelectedPrdLabeling(pTestBrownSntncs, CONFIDENECE_ONLY, pLog);
     mout("\nLabeling Brown test data is done!\n", cout, pLog);  
   ...
   if (pRemoveLabeled != 0)
    vDataUsedSofar += vAllUSize - pUnlabeledSamples.size() - vPool.size();
   else
   {
    if (pPoolUsage > 0)
     vDataUsedSofar += vPool.size();
    else 
     vDataUsedSofar += pUnlabeledSamples.size();
    }
 ...
 * 
 *  Note that, pCLSelMethod is only used for labeling test sets not unlabeled 
 *  data, because this method is not based on common training set.
 * 
 *  For explanation of parameters, refer to the option list of the main
 *  function above
 * 
...
void coTrainSeparate(vector<ZMEClassifier> &pZMEClassifiers,
                     ...
                     ofstream &pLog)
{
 unsigned short vSeedSize = pTrainingSamples.size();
 unsigned vAllUSize = pUnlabeledSamples.size();
 ...
 vector<Sample *> vTrainingSamples1 = pTrainingSamples;
 vector<Sample *> vTrainingSamples2 = pTrainingSamples;
 vector<Sample *> vUnlabeledSamples1 = pUnlabeledSamples;
 vector<Sample *> vUnlabeledSamples2 = pUnlabeledSamples;
 
 // freeing memory for original training and unlabeled set
 pTrainingSamples.clear();
 pUnlabeledSamples.clear();
 
 // pools for each view
 vector<Sample *> vPool1;
 vector<Sample *> vPool2;
 ...
 while ((vUnlabeledSamples1.size() > 0) && (vUnlabeledSamples2.size() > 0))
 {
  ...
  if (vUnlabeledSamples1.size() < pPoolSize)
   vPoolSize1 = vUnlabeledSamples1.size();
  vPool1.insert(vPool1.end(),
                vUnlabeledSamples1.begin(), 
                vUnlabeledSamples1.begin() + vPoolSize1);
  vUnlabeledSamples1.erase(vUnlabeledSamples1.begin(), 
                           vUnlabeledSamples1.begin() + vPoolSize1);
   
  if (vUnlabeledSamples2.size() < pPoolSize)
   vPoolSize2 = vUnlabeledSamples2.size();
  vPool2.insert(vPool2.end(),
                vUnlabeledSamples2.begin(), 
                vUnlabeledSamples2.begin() + vPoolSize2);
  vUnlabeledSamples2.erase(vUnlabeledSamples2.begin(), 
                           vUnlabeledSamples2.begin() + vPoolSize2);
  ...
  while (1)
  {
   ...
   if (pRemoveLabeled)
    vAddedCount2 = moveToTraining(vTrainingSamples2, vUnlabeledSamples1, vPool1,
    ...
    //    for consistency with other setups and also preventing exceptions
    //    on inapproperiate parameters.
    vAddedCount2 = addToTraining(vTrainingSamples2, vPool1, vSeedSize);
   ...
   if (pRemoveLabeled)
    vAddedCount1 = moveToTraining(vTrainingSamples1, vUnlabeledSamples2, vPool2,
    ...
    vAddedCount1 = addToTraining(vTrainingSamples1, vPool2, vSeedSize);
   ...
   if ((vAddedCount1 != 0) && (vAddedCount2 != 0))
   {
    ...
    mout("\nTraining " + intToStr(cntIteration) + "th classifier of view 1 (" + 
         getLocalTimeStr() + ") ...\n", cout, pLog);
    pZMEClassifiers[0].train(vTrainingSamples1, VIEW_1, VIEW_2, false, 0, pMEIterations, 
    ...
    mout("\nTraining " + intToStr(cntIteration) + "th classifier of view 2 (" + 
         getLocalTimeStr() + ") ...\n", cout, pLog);
    pZMEClassifiers[1].train(vTrainingSamples2, VIEW_2, VIEW_1, false, 0, pMEIterations, 
                             pMEPEMethod, pGaussian, false, "", pLog);
   ...
   if (pRemoveLabeled != 0)
    vDataUsedSofar += vAllUSize - pUnlabeledSamples.size() - vPool2.size();
   else
   {
    if (pPoolUsage > 0)
   ...
   if (pRemoveLabeled != 0)
    vDataUsedSofar += vAllUSize - pUnlabeledSamples.size() - vPool1.size();
   else
   {
    if (pPoolUsage > 0)
 ...
 */ 
unsigned int addToTraining(vector<Sample *> &pTrainingSamples,
                           vector<Sample *> &pPool,
                           unsigned int pSeedSize)
{                            
 if (pTrainingSamples.size() != pSeedSize)
  pTrainingSamples.resize(pSeedSize);
  
 pTrainingSamples.insert(pTrainingSamples.begin() + pSeedSize, 
 ...
 *  training set.
 */ 
unsigned int moveToTraining(vector<Sample *> &pTrainingSamples,
                            vector<Sample *> &pUnlabeledSamples,
                            vector<Sample *> &pPool,
...
{                            
 ...
 switch (pSelection)
 {
  ...
  case (0):
   pTrainingSamples.insert(pTrainingSamples.end(), pPool.begin(), pPool.end());
  ...
  case (1): 
   vSelected = selectAndMoveAgreed(pPool, pTrainingSamples, 
  ...
  case (2): 
   if (pCoTrainingMethod == 1)
    vSelected = selectAndMoveConfident(pPool, pTrainingSamples,
                                       pProbThreshold, pNumberThreshold, 
                                       pConvergenceThreshold, pLogSelection,
                                       pLogFileName, pLog);
   else if (pCoTrainingMethod == 2)
    vSelected = selectAndMove(pPool, pTrainingSamples, pSelectionView, 
 ...
 if (pSelection != 0) 
  switch (pPoolUsage)
  {
   // If no pool is used and no labeled sample was selected, co-training
   // has converged, so clear the remaining samples to end it.
   case (0):
    if (vSelected == 0)
     pPool.clear();    
    break;
   ...
   // wandering around some difficult samples)
   case (1):
    if (vSelected == 0)
     pPool.clear();    
    break;
     
   // remove away the unselected samples
   case (2):
    pPool.clear();    
    break;

   // Return (add) unselected data to the begining of unlabeled samples
   // (useful when unlabeled data is sorted) if some samples was selected
   // above, UNLESS there was no sample selected (vSelected == 0);
   ...
   // wandering around some difficult samples), otherwise the remaining 
   // data may not be useful and will cause infinit loop, so do nothing.
   case (3):
    if (vSelected == 0)
    {
     if (pUnlabeledSamples.size() >= (pPool.size() / 2))
      pUnlabeledSamples.insert(pUnlabeledSamples.end(), pPool.begin(), pPool.end());
     }
    else
     pUnlabeledSamples.insert(pUnlabeledSamples.begin(), pPool.begin(), pPool.end());
     
    pPool.clear();
    break;

   // return (add) unselected data to the end of unlabeled samples
   // (useful when unlabeled data is sorted)
   // If there was no sample selected (vSelected == 0), see if the size
...
///---------------------------------------------------------------------
/**
 *  selects pNumberThreshold top most agreed-upon labeling of samples and
 *  return the number of selected samples
 * 
 *  Selection can be logged into a file (including the surface form of the
 *  belonging sentence and predicted label) based on the value of 
 *  pLogSelection:
 *  0: do not log
 *  1: log only selected data
 *  2: log selected and filtered data
 */ 
unsigned int selectAndMoveAgreed(vector<Sample *> &pPool, 
                                 vector<Sample *> &pTrainingSamples, 
                                 unsigned short pNumberThreshold,
                                 unsigned short pConvergenceThreshold,
                                 unsigned short pLogSelection,
                                 ...
                                 ofstream &pLog)
{
 ...
 // sorts to agreed samples come first
 sort(pPool.begin(), pPool.end(), isAgreedLabel);
 ...
 // recognizing the last agreed-upon label
 for (; cntSortedPool > 0; cntSortedPool--)
  if (pPool[cntSortedPool - 1]->isPrdArgLabelsAgreed())
   break;
 ...
 // is no room to further improvment. So, no sample is selected.
 //
 ...
 if (vSelectedCount != 0)
 {
  pTrainingSamples.insert(pTrainingSamples.end(), 
  ...
  mout("No more room to improve (" + intToStr(pPool.size()) +
       " remaining samples in pool discarded)\n", cout, pLog);
...
///---------------------------------------------------------------------
/**
 *  selects pNumberThreshold top most probable labeling of samples if 
 ...
 * 
 */ 
unsigned int selectAndMoveConfident(vector<Sample *> &pPool, 
                                    vector<Sample *> &pTrainingSamples, 
                                    double pProbThreshold, 
                                    unsigned short pNumberThreshold,
                                    unsigned short pConvergenceThreshold,
                                    ...
                                    ofstream &pLog)
{
 ...
 sort(pPool.begin(), pPool.end(), isMoreLikelyLabel);
 ...
 for (; cntSortedPool > 0; cntSortedPool--)
  if ((pPool[cntSortedPool - 1]->getPrdArgLabelProb(0) >= pProbThreshold) &&
      (pPool[cntSortedPool - 1]->getPrdArgLabelProb(0) != 0))
   break;
 ...
 // If the number of selected samples in this iteration is less than
 // a certain amount, it means that co-training has converged and there
 // is no room to further improvment. So, no sample is selected.
 //
 // Note that this will also discard all labeling with probability of 0 
 if (cntSortedPool <= pConvergenceThreshold)
 ...
 if (vSelectedCount != 0)
 {
  pTrainingSamples.insert(pTrainingSamples.end(), 
                         pPool.begin(), pPool.begin() + vSelectedCount); 
  vMinProbability = pPool[vSelectedCount - 1]->getPrdArgLabelProb(0);
  vMaxProbability = pPool[0]->getPrdArgLabelProb(0);
 ...
 *  see selectAndMoveAgreed() for more comments
 */ 
unsigned int selectAndMove(vector<Sample *> &pPool, 
                           vector<Sample *> &pTrainingSamples, 
                           unsigned short pSelectionView,
                           double pProbThreshold, 
                           unsigned short pNumberThreshold,
                           ...
                           ofstream &pLog)
{
 ...
 // identify the selection boundary
 for (; cntSortedPool > 0; cntSortedPool--)
  if ((pPool[cntSortedPool - 1]->getPrdArgLabelProb(pSelectionView) >= pProbThreshold) &&
      (pPool[cntSortedPool - 1]->getPrdArgLabelProb(pSelectionView) != 0))
   break;
 ...
 if (vSelectedCount != 0)
 {
  pTrainingSamples.insert(pTrainingSamples.end(), 
                         pPool.begin(), pPool.begin() + vSelectedCount); 
  vMinProbability = pPool[vSelectedCount - 1]->getPrdArgLabelProb(pSelectionView);
  vMaxProbability = pPool[0]->getPrdArgLabelProb(pSelectionView);
